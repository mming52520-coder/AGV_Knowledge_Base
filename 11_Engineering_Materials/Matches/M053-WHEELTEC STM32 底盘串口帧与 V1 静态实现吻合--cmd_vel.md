...
right_entity: "agv:ros:interface:cmd-vel"
target_snapshot: "code-v1"
relation: "specifies_transport_for"
match_score: 96
match_probability: 0.96
match_status: "accepted"
match_reasons: ["手册与节点链共同支持 /cmd_vel 到 STM32 的速度控制传输。"]
...
# WHEELTEC STM32 底盘串口帧与 V1 静态实现吻合 → /cmd_vel

## 匹配结论

- 左侧工程结论：[[11_Engineering_Materials/Entities/WHEELTEC STM32 底盘串口帧与 V1 静态实现吻合|WHEELTEC STM32 底盘串口帧与 V1 静态实现吻合]]
- 右侧代码实体：[[03_ROS/Interfaces/cmd-vel|/cmd_vel]]
- 关系：`specifies_transport_for`
- 匹配概率：**96%**
- 匹配状态：`accepted`
...
- 手册与节点链共同支持 /cmd_vel 到 STM32 的速度控制传输。
